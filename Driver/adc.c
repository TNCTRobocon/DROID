...
void adc_setup(){
    ...
    ADCON1bits.SIMSAM = true;   //CH0とCH1を同時サンプリングをする
    ...
    ADCON2bits.BUFS = true;     //とりあえずtrue(バッファステータスビット？)
    ADCON2bits.SMPI = 0b1111;   //サンプル時間の選択(最大値)(計算してない)
    ADCON2bits.BUFM = false;    //意味不明(とりあえずfalse)バッファモードの選択？
    ...
    ADPCFGbits.PCFG1 = true;    //AN1をアナログ入力に設定
    
    ADCHSbits.CH0NA = false;    //チャネル0の負の入力はVref-3
    ADCHSbits.CH0SA = 0b0001;   //正の入力はRA1
    
    IFS0bits.ADIF = false;      //AD変換による割り込みフラッグリセット
    
    IEC0bits.ADIE = true;       //AD変換割り込み許可
